  ...
  Selectpicker.prototype = {
    ...
    clickListener: function() {
      ...
      this.$menu.on('click', 'li a', function(e) {
        ...
        // add by raye
        var aggReportSelectDiv = $(this).parents('.aggReport-select-div');
        var app = aggReportSelectDiv.find('input[name=app]').val();
        var dup = aggReportSelectDiv.find('input[name=dup]').val();
        var reportTag = aggReportSelectDiv.find('input[name=report_tag]').val();
        var developer = $(this).attr('data-tokens');
        var method = aggReportSelectDiv.find('input[name=method]').val();
        ...
        $.post(
          ...
          function (data) {
            ...
            if (dup == "True") {
              var hiddenDiv = aggReportSelectDiv.parents("#agg-title-"+reportTag).parent();
              hiddenDiv.hide();
              // set the administrator log
              var selectHiddenDiv = $("#agg-log-"+reportTag);
              selectHiddenDiv.addClass("active");
              selectHiddenDiv.find(".state-icon")
                  .removeClass().
                  addClass('state-icon glyphicon glyphicon-check');

              // set select menu hidden
              $(".select-hidden").hide();
              var developer_name = aggReportSelectDiv.find('li.selected a span.media-body').text();
              $(".dup-assignee").html("<a href='#' style='margin-right: 5px;'>"+developer_name+"</a>");
            } else {
              var hiddenDiv = aggReportSelectDiv.parents("#report-title-"+reportTag).parent();
              hiddenDiv.hide();
              // set the administrator log
              var selectHiddenDiv = $("#report-log-"+reportTag);
              selectHiddenDiv.addClass("active");
              selectHiddenDiv.find(".state-icon")
                  .removeClass().
                  addClass('state-icon glyphicon glyphicon-check');
